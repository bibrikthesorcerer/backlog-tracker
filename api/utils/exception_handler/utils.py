...
def custom_exception_handler(
    ...
    if isinstance(exception, Http404):
        ...
        if (
            ...
            details = {
                field_name[:-1]: [  # remove dot from the end 
                    {
    ...
    #         debug_message="Use valid refresh token",
    #     )
        ...
        exception = exceptions.AuthenticationFailed(    # why not to use exception.message? linter pre-commits?
        ...
        debug_message = exception.__str__()
        exception = exceptions.ForbiddenError()
        exception.debug_message = debug_message
    ...
    # handle ServiceObjects library  exceptions
        ...
        details = {}
        for field, exception_details in exception.errors_dict.items(): # each field's accumulated errors
...
def drf_exception_response(
    ...
    response = Response(exception.response_dict, status=exception.response_status)
    return response
